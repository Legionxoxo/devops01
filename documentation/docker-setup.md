# Docker Setup Guide for devops01 API

This guide explains how to run the devops01 API using Docker with different configurations for development and production environments.
...
docker exec devops01-app-dev npm run db:migrate

# Open Drizzle Studio (inside running container)
docker exec devops01-app-dev npm run db:studio
```
...
docker exec devops01-neon-local psql -U neon -d neondb
```
...
docker exec devops01-app-prod npm run db:migrate

# Or run one-time migration container
docker run --rm -it --env-file .env.production devops01-app npm run db:migrate
```
...
docker exec devops01-app-prod npm run db:studio
```
...
docker logs devops01-neon-local
...
# Manual migration
docker exec -it devops01-app-dev npm run db:generate
docker exec -it devops01-app-dev npm run db:migrate
```
